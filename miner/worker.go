...
func (w *worker) commitBundle(env *environment, txs types.Transactions, interrupt *int32, interruptCh chan struct{}) error {
	var (
...
func (w *worker) fillTransactions(ctx context.Context, interrupt *int32, env *environment, interruptCh chan struct{}) {
	...
	if w.flashbots.isFlashbots {
		...
		if err := w.commitBundle(env, bundleTxs, interrupt, interruptCh); err != nil {
			return
		}
